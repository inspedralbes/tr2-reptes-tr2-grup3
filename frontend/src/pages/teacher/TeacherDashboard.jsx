...
const TeacherDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const workshopsRes = await fetch(`${API_URL}/teachers/my-workshops`, { headers });
      if (workshopsRes.ok) {
        const data = await workshopsRes.json();
        setWorkshops(data);
        
        // Para cada taller, obtener las sesiones de hoy/próximas
        const today = new Date().toISOString().split('T')[0];
        const allSessions = [];
        
        for (const w of data) {
          const sessionsRes = await fetch(`${API_URL}/sessions/${w.edition_id}`, { headers });
          if (sessionsRes.ok) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-100 p-4">
        <p className="text-gray-500 text-center mt-8">Cargando tus talleres...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-700 text-white p-6">
        ...
        <p className="text-green-100 mt-1">Hola, {user?.full_name || "Profesor"}</p>
      </div>
              ...
              {todaySessions.map((session) => (
                <button
                  ...
                  className={`w-full text-left bg-white rounded-xl shadow-sm p-4 border-l-4 ${
                    isToday(session.date) ? 'border-green-500' : 'border-gray-300'
                  }`}
                >
        ...
        {workshops.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-8 text-center">
            ...
            <p className="text-gray-500">
              Cuando el administrador te asigne como referente de un taller, aparecerá aquí.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-gray-800">🎓 Tus Talleres</h2>
            {workshops.map((workshop) => (
              <div 
                key={workshop.id}
                className="bg-white rounded-xl shadow-sm overflow-hidden"
              >
                <div className={`${getDayColor(workshop.day_of_week)} text-white px-4 py-2 text-sm font-medium`}>
                  {getDayLabel(workshop.day_of_week)} - {workshop.start_time} a {workshop.end_time}
                </div>
                <div className="p-4">
                  ...
                  <p className="text-gray-500 text-sm mt-1">
                    {workshop.is_main_referent ? "📌 Referente Principal" : "👥 Referente de Apoyo"}
                  </p>
                  
                  {/* Botones de acción - Mobile First */}
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <button
                      onClick={() => navigate(`/teacher/workshop/${workshop.edition_id}/evaluate`)}
                      className="bg-blue-600 text-white py-3 px-4 rounded-lg font-medium text-center hover:bg-blue-700 transition"
                    >
                      ⭐ Evaluar
                    </button>
                    <button
                      onClick={() => navigate(`/admin/catalog/${workshop.edition_id}`)}
