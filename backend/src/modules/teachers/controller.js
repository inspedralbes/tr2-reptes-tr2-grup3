...
const assignTeacher = async (req, res) => {
  ...
  if (!workshop_edition_id || !teacher_id) {
    return res.status(400).json({ error: 'workshop_edition_id i teacher_id són requerits' });
  }
...
const getMyWorkshops = async (req, res) => {
  return res.status(410).json({ error: 'This feature is no longer supported as teachers do not have accounts.' });
};
...
const getByCenter = async (req, res) => {
  try {
    ...
    if (!schoolId) {
      return res.status(400).json({ error: 'School ID not found in token' });
    }
...
const update = async (req, res) => {
  try {
    ...
    if (check.rows.length === 0) {
      return res.status(404).json({ error: 'Professor no trobat o no pertany a la teva escola' });
    }
